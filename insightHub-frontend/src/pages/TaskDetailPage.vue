...
const task = ref<ResearchTask | null>(null); const report = ref<Report | null>(null); const streamingReport = ref(''); const citations = ref<Citation[]>([]); const events = ref<TaskEvent[]>([]); const activeTab = ref('activity'); const loading = ref(true); const actionLoading = ref(''); const connected = ref(false); let eventSource: TaskEventSource | null = null; let reportFlushTimer: number | undefined; let pendingReport = ''; let latestEventNo = 0
...
const reportContent = computed(() => report.value?.markdownContent || streamingReport.value)
const eventTitle = (event: TaskEvent) => ({ TASK_STARTED: '任务开始执行', PLAN_CREATED: '研究计划已生成', NODE_STARTED: `${event.node || '研究节点'} 开始`, NODE_COMPLETED: `${event.node || '研究节点'} 完成`, TOOL_CALLED: '调用研究工具', TOOL_COMPLETED: '工具返回结果', REPORT_DELTA: '报告片段已生成', TASK_PAUSED: '任务已暂停', TASK_COMPLETED: '研究内容完成', TASK_FAILED: '任务执行失败', TASK_RESULT: '任务结果已写入' }[event.type] || event.type)
...
async function loadEvents() {
  ...
  latestEventNo = rows.reduce((max, item) => Math.max(max, item.eventId || 0), 0)
...
function connectEvents() {
  ...
  const fromEventNo = latestEventNo
...
function handleEvent(event: TaskEvent) {
  if (event.eventId > 0 && event.eventId <= latestEventNo) return
  latestEventNo = Math.max(latestEventNo, event.eventId || 0)
  ...
  if (event.type === 'REPORT_DELTA' && typeof event.data?.delta === 'string' && !report.value) {
    pendingReport += event.data.delta
    if (reportFlushTimer === undefined) {
      reportFlushTimer = window.setTimeout(() => {
        streamingReport.value += pendingReport
        pendingReport = ''
