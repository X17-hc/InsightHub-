...
import { formatDate, qualityStatusMeta } from '@/utils/display'
import { PerformativeIsland, TokenStream } from '@/integrations/performative'

const props = defineProps<{ report: Report | null; streamingContent: string; status: TaskStatus; workspaceId: string; taskId: string; versionLoading?: boolean }>()
...
const qualityLabel = (value: unknown) => typeof value === 'string' && value in qualityStatusMeta
  ? qualityStatusMeta[value as keyof typeof qualityStatusMeta].label
  : String(value || '未评估')
...
<template>
  <div class="section-title report-title"><div><h2>研究报告</h2><p v-if="report">版本 {{ report.version }} · 更新于 {{ formatDate(report.updatedAt) }}</p><p v-else-if="streamingContent">正在接收报告片段。</p><p v-else>基于已核验证据生成。</p></div><div><a-tag v-if="report" :color="qualityStatusMeta[report.qualityStatus]?.color">{{ qualityStatusMeta[report.qualityStatus]?.label || report.qualityStatus }}</a-tag><StatusTag v-if="report" :status="report.status === 'READY' ? 'COMPLETED' : report.status"/><StatusTag v-else-if="streamingContent" status="GENERATING"/></div></div>
  ...
  <a-drawer v-model:open="historyOpen" title="报告历史版本" placement="right" width="360"><a-list :data-source="versions"><template #renderItem="item"><a-list-item><a-button type="link" :loading="versionLoading" :disabled="versionLoading" @click="emit('selectVersion', item.version)">版本 {{ item.version }} · {{ item.title }} · {{ qualityLabel(item.qualityStatus) }}</a-button></a-list-item></template></a-list></a-drawer>
</template>
