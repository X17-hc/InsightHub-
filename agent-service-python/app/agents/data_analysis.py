...
def _emit(event: dict[str, Any]) -> None:
    ...
    except Exception:  # noqa: BLE001 - custom stream 不可用时仍由 State 持久化事件
        logger.debug("custom stream writer is unavailable", exc_info=True)
...
def data_analysis(state: ResearchState) -> dict[str, Any]:
    ...
    if not evidence:
        ...
        _emit(done)
        return {"step_count": step, "analysis_artifacts": [], "events": [start, done]}
    try:
        ...
        _emit(failed)
        return {"step_count": step, "status": "FAILED", "errors": [{"code": "SANDBOX_UNAVAILABLE", "message": message}], "events": [start, failed]}
        ...
        _emit(failed)
        return {"step_count": step, "status": "FAILED", "errors": [{"code": "SANDBOX_FAILED", "message": message}], "events": [start, failed]}
    ...
    _emit(done)
    return {"step_count": step, "analysis_artifacts": artifacts, "events": [start, done]}
